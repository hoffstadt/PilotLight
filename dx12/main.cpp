...
#define S_UPLOAD_HEAP_SIZE 1024u*1024u*4u
#define S_DESCRIPTOR_HEAP_SIZES 10u
#define S_CBUF_COUNT 1
...
// [SECTION] general variables
//-----------------------------------------------------------------------------
struct FrameContext { ID3D12CommandAllocator* commandAllocator; size_t fenceValue;};
static FrameContext                g_frameContext[S_FRAME_COUNT];
static FrameContext*               g_currentFrameContext;
static unsigned                    g_backBufferIdx;
...
static size_t                      g_uploadHeapSize = S_UPLOAD_HEAP_SIZE;
...
static unsigned                    g_rtvHeapSize = S_DESCRIPTOR_HEAP_SIZES; 
static unsigned                    g_dsvHeapSize = S_DESCRIPTOR_HEAP_SIZES;
static unsigned                    g_cbvHeapSize = S_DESCRIPTOR_HEAP_SIZES;
static unsigned                    g_srvHeapSize = S_DESCRIPTOR_HEAP_SIZES;
...
static void create_descriptor_heaps();
static void create_command_queue();
static void create_command_allocators();
static void create_command_lists();
static void create_syncronization_primitives();
static void create_swapchain();
static void create_render_targets();
static void create_depth_resources();
static void create_upload_heap();
static void cleanup();

//-----------------------------------------------------------------------------
// [SECTION] example specific setup function declarations
//-----------------------------------------------------------------------------
static void create_uniform_heap();
...
create_uniform_heap()
{
    ...
    desc.SizeInBytes = 65536u*S_CBUF_COUNT*S_FRAME_COUNT;
...
begin_frame()
{
    ...
    g_currentFrameContext = &g_frameContext[nextFrameIndex % S_FRAME_COUNT];
    size_t fenceValue = g_currentFrameContext->fenceValue;
    if (fenceValue != 0) // means no fence was signaled
    {
        g_currentFrameContext->fenceValue = 0;
        g_fence->SetEventOnCompletion(fenceValue, g_fenceEvent);
...
{
    g_currentFrameContext->commandAllocator->Reset();
    g_commandList->Reset(g_currentFrameContext->commandAllocator, nullptr);
}
...
begin_pass()
{
    ...
    g_backBufferIdx = g_swapChain->GetCurrentBackBufferIndex();
    ...
    toTargetBarrier.Transition.pResource = g_mainRenderTargetResource[g_backBufferIdx];
    ...
    g_commandList->OMSetRenderTargets(1, &g_mainRenderTargetDescriptor[g_backBufferIdx], FALSE, &g_depthStencilView);
    g_commandList->ClearRenderTargetView(g_mainRenderTargetDescriptor[g_backBufferIdx], clear_color, 0, nullptr);
...
end_pass()
{
    ...
    toPresentBarrier.Transition.pResource = g_mainRenderTargetResource[g_backBufferIdx];
...
present()
{
    ...
    g_currentFrameContext->fenceValue = nfenceValue; 
}
